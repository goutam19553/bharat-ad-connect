...
const AvatarModel = ({ speaking }: { speaking: boolean }) => {
  ...
  useFrame(() => {
    if (avatarRef.current) {
      if (speaking) {
        avatarRef.current.rotation.y += 0.01; // faster rotation while speaking
        avatarRef.current.position.y = Math.sin(Date.now() * 0.01) * 0.05; // subtle bob
      } else {
        // Idle slow rotation when not speaking
...
const HumanAIAvatar = () => {
  ...
  const startSpeech = () => {
    if (speaking) return; // prevent multiple triggers

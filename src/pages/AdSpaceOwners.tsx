...
const AdSpaceOwners = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 dark:text-white px-4 py-10 space-y-12 pt-24">
      {/* Padding top 6rem to avoid navbar overlap */}

      <section className="text-center space-y-4">
        <h1 className="text-4xl font-bold">Partner with Us</h1>
        <p className="text-lg max-w-2xl mx-auto">
          List your property, wall, or building and start earning with India's most advanced physical ad network.
        </p>
      </section>

      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {benefits.map((benefit, index) => (
          <div key={index} className="p-6 rounded-xl shadow-md bg-gray-100 dark:bg-gray-800">
            {benefit.icon}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {spaceTypes.map((space, index) => (
            <div key={index} className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4">
              <img src={space.image} alt={space.title} className="w-full h-48 object-cover rounded-md mb-4" />
      ...
      <section className="space-y-6">
        <h2 className="text-3xl font-bold text-center">How It Works for Owners</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, index) => (
            <div key={index} className="p-6 bg-gray-100 dark:bg-gray-800 rounded-xl text-center">
              <div className="text-4xl font-bold text-bharat-green dark:text-bharat-saffron">{step.number}</div>
